...
export function LineupLab() {
  const { payload: p, ui, toggleSlot, swapInSlot, applyAct, setAce, save, reset, dirty } = useStore();
    ...
    <div className="page">
      <div className="c6" style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        <section className="tile open">
          <div className="th"><span className="lbl">Lineup · {dirty ? <span className="red">unsaved changes</span> : 'saved'}</span><span className="mut">Tap a tile to see swaps</span></div>
          <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap', alignItems: 'flex-end' }}>
            <div><div className="big num">{proj}</div><span className="mut">Projected pts</span></div>
            <div><div className="h2 num">{money(room)}</div><span className="mut">Bank of {money(p.budget)}</span></div>
            <div><div className="h2 num">{Math.min(99, Math.round(18 + proj / 9))}%</div><span className="mut">League win chance</span></div>
          </div>
          <div className="lineup">{l.drivers.map((id) => tile(entity(p, id)!, false))}{tile(entity(p, l.ctor)!, true)}</div>
          <div className="th">
            <button type="button" className="cta" disabled={!dirty} onClick={save}>{dirty ? 'Keep this what-if' : 'No changes'}</button>
            <button type="button" className="ghost" onClick={reset}>Reset</button>
            <span className="mut">Saving to your real team goes through the same server rules as the app, and is disabled after lock.</span>
          </div>
        </section>
